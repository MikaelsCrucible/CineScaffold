...
- Core 0.8.39 / Semantic Parser 0.21 将动作阶段的时间权威收口到 `timeline.events`：Semantic AI 只输出事件起止秒数一次，`subject_motion` 必须引用事件，Core 再为下游规范化 Brief 确定性派生动作区间。Semantic 修正不再限定一次：初稿之后持续使用上一版完整草稿、当前确定性诊断和 ReviewPacket 进行修正，直到通过契约，或独立的 50 秒 Semantic 总墙钟预算/全管线成本上限到达。每个 Provider 请求都被剩余 Semantic 时间截断，不占用 Planning 的独立时限。每轮原始 Semantic 草稿及诊断保存在运行目录内部 `semantic_diagnostics/`，不进入用户交付 artifact map。
...
- Toolkit 0.47 不再把单次相遇误解为整个短区间持续接近。双方各自的 Semantic 速度会穿过中央关系路径点保持不变，避免为了在区间首、中、末都接近而同步骤降速度。
