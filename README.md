...
- Semantic 每次解析固定执行两次 Provider 请求：第一轮生成初稿，第二轮依据原文、完整初稿、确定性诊断和版本化 `ReviewPacket` 做独立审查，并返回完整替换 JSON。Core 0.8.33 会根据稳定错误码/字段路径、初稿结构以及原文与初稿中的概念提示召回最多五条重点规则；`confirmed_error` 与 `review_risk` 严格区分，提示词只负责选择审查问题，绝不直接生成动作、方向、目标、关系或时间答案。Core 0.8.37 又把来源证据、uncertainty resolution、动作组合、关系事件、摄影机目标、时间与引用完整性登记为代码拥有的跨字段契约，并自动注入两轮 Prompt；同一契约 ID 必须出现在 JSON Schema，回归测试会阻止遗漏。Core 0.8.38 进一步逐项审计 Structured Output 的全部字段组，并要求每个实体状态区间完整覆盖镜头、叙事载运具有同期载体位移；Planning 的每个已注册方法则由代码注册表注入方法语义、调用前提、结果与下一步动作，Route Context 直接提供合法的 `canonical_source_ref`，Agent 不再猜内部路径。完整契约始终有效，实际召回规则、证据、目录版本与 SHA-256 写入 Brief provenance。这是有上限的一次修正，不是网络重试，也不增加第三次模型调用；第一轮若在取得可用 JSON 前发生超时或 Provider 错误，不会盲目重复请求。第二轮仍不满足 Schema、类型字段一致性、引用或时间区间约束时以 `semantic_contract_invalid` 明确失败。两次请求分别报告进度和费用，该流程与可选 Provider thinking mode 相互独立。
