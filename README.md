...
- Cinematic Brief v0.8 先区分主体静态/动态场景；每个实体的独立状态区间并集必须从 `0` 连续覆盖到总时长，不能把未描述的空窗交给执行层静默解释成“可见且静止”。完整覆盖只要求状态有解释，不要求实体从头可见：中途出现使用进入前 hidden 状态及末端 `becomes_visible`，Planning 从第一次显隐转变反推初始可见性。事件可以位于镜头内任意数值区间，九种类型化时间关系表达相接、先后、包含、同始/同终和重叠；空间关系的 `throughout/at_start/at_midpoint/at_end` 只描述“所引用事件内部”的完整区间、中央采样或边界，不是四个全局时间点。全流程明确区分“主体状态发生变化”“主体发生空间位移”和“摄影机发生运动”：运镜不能把主体静态场景变成动态，也不能触发主体运动可读性策略；只有 `path_move/carried` 才属于主体空间运动。所有独立位移统一为 `path_move`；公转只由 `relative_to_target + target_id + circle/ellipse` 表达，载运只由 `carried + carrier_id` 表达，且叙事要求“带走”时载体必须在完整载运区间拥有同期自主世界位移。普通人物/车辆贴地由 Planning 根据地面、代理类型和类型化动作确定性生成，三者都不再要求 Semantic 重复提交关系。局部互动必须物化为可观察的旋转/尺度变化，不能用静止轨道假装完成；抛物线路径会物化为带弧顶的关键点。稳定 `motion_id` 必须绑定对应的类型化动作来源，不能通过改写 `source_ref` 绕过验证。
- 当前版本只接受 Cinematic Brief v0.8、Constraint Plan v0.2 与 Scene IR v0.2。旧版中间产物不会被猜测、补字段或迁移；需要重新生成。已经交付的 MP4/GLB 仍是普通成品文件，不受这条版本边界影响。
- Semantic 首轮生成草稿，之后依据原文、上一版完整草稿、确定性诊断和版本化 `ReviewPacket` 持续 revision，直到结构与一致性契约通过，或全管线 Provider 成本上限/独立 Semantic 总时限到达。默认 `semantic_timeout=50`，它是整个 Semantic 阶段的墙钟预算；每次请求使用当时剩余时间作为截止，不占用 Planning 的独立预算。初始 Provider 请求若未取得 JSON 就超时或报错，仍保留类型化 Provider 失败，不伪装成契约 revision。每轮 Provider 进度和费用单独记录，草稿、诊断与 ReviewPacket 写入运行目录内部 `semantic_diagnostics/`，但不进入用户交付 artifact map。Core 0.8.39 还将动作时间收口为单一权威：Semantic AI 只在 `timeline.events` 填写起止秒数，`subject_motion` 通过必填 `timeline_event_id` 引用，Core 再确定性派生下游 Brief 的动作区间。旧的重复时间模型输出不兼容。
...
也可使用 `--semantic-thinking-mode`、`--semantic-reasoning-effort` 和 `--max-tokens` 临时覆盖。只提供“10 秒以内”等时长上限时，Semantic Parser 保留原范围，并确定性选择上限作为本次时长；完全未提供时长才使用 10 秒缺省值。
...
Provider HTTP 边界使用无网络模拟回归覆盖认证拒绝（`401`）、限流（`429`）、余额不足（`402`）、服务繁忙（`503`）、网络失败与超时；HTTP 拒绝保留类型化状态码且底层不会自动重试。HTTP 响应按当前 Semantic 剩余总墙钟时间增量读取，DeepSeek 非流式空行保活可以继续传输，但不能刷新或延长 `semantic_timeout`。Pipeline Summary 额外携带稳定、无 Secret 的 `failure_code`，供宿主区分过载、限流、认证、余额、网络与超时；已获得的 Semantic 草稿与契约诊断只保留在内部诊断产物。模型响应前的明确请求拒绝会结算零费用，超时与 5xx 继续保留账单不确定性；金额越线另由 Workflow 测试确认 Semantic 已产生的费用先记录，随后停止新的 revision 或 Planning 请求。
