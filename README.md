...
- Semantic 每次解析固定执行两次 Provider 请求：第一轮生成初稿，第二轮依据原文、完整初稿和确定性诊断做独立审查，并返回完整替换 JSON。这是有上限的一次修正，不是网络重试；第一轮若在取得可用 JSON 前发生超时或 Provider 错误，不会盲目重复请求。第二轮仍不满足 Schema、类型字段一致性、引用或时间区间约束时直接失败，不再用多数表决、时间关系改名或场景动态改写掩盖错误。两次请求分别报告进度和费用，最终 provenance 聚合两次 usage。该审查流程始终启用，与可选的 Provider thinking mode 相互独立。
