...
- Cinematic Brief v0.6 先区分主体静态/动态场景；动态场景按实体记录稀疏、独立的动作区间，并用类型化时间关系表达相接、先后、包含和重叠，不再按全场事件数量机械等分总时长。全流程明确区分“主体状态发生变化”“主体发生空间位移”和“摄影机发生运动”：运镜不能把主体静态场景变成动态，也不能触发主体运动可读性策略；只有 `path_move/carried` 才属于主体空间运动。所有独立位移统一为 `path_move`；公转只由 `relative_to_target + target_id + circle/ellipse` 表达，不再同时提交 `orbit` kind、`orbit_around` direction 或重复 Skeleton Relation。闭合路径的类型化目标本身就是几何证据，不要求重复填写自然语言方向；目标缺失时只允许从同一事件的唯一公转关系确定性补齐。局部互动必须物化为可观察的旋转/尺度变化，不能用静止轨道假装完成；抛物线路径会物化为带弧顶的关键点。稳定 `motion_id` 必须绑定对应的类型化动作来源，不能通过改写 `source_ref` 绕过验证。
