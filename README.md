...
Agent 协议由 Runner 确定性约束：闭集参数直接进入 Tool Schema 枚举；模型开始时只看得到能力工具，读取后该工具会从后续请求移除；`commit_ready=true` 后所有 Candidate 工具都会移除，只能结构化终止。同一 revision 的相同 inspect 会被拒绝；实体和摄影机的每个状态通道最多存在一条 Track。同一 ID 的 remove + upsert 是原子替换；Agent 推断或自选的数值不得伪装成 hard constraint，hard constraint 的明确来源还必须与约束类型兼容。“远处 / 后景”至少需要 `depth_order`，不能只用欧氏距离冒充画面深度。实体通过 `ground_interaction` 显式声明地面关系；缺省禁止穿地，只有 Brief 中明确的埋入/地下语义才能获得穿地豁免，且完整代理几何保持不被削短。实体建立后代理体类型和自身轴向不可更换。Toolkit v0.9 已实现 `world/local/target_relative/camera` 参考系：闭合路径会补齐末段，目标相对运动按当前帧 Transform 递归合成，引用循环原子拒绝，相对距离在全部冻结帧验证，再统一烘焙为 Scene IR 的 parent-local TRS；Agent 不应手算复合世界轨迹。默认请求上限为 24；Checkpoint 只在当前 Candidate revision 真正变化时写入，Toolkit 版本变化会拒绝旧 checkpoint。
...
相对运动能力已用嵌套回归覆盖：地球通过 `target_relative` 闭合路径围绕太阳，月亮再通过第二层 `target_relative` 闭合路径围绕正在运动的地球；世界求值、父级局部 IR 编译、父级旋转合成、摄影机相对实体、循环拒绝及全帧轨道半径验证均有离线测试。该能力是通用参考系系统，不包含太阳系专用代码。

同一太阳—地球—月亮样本已完成真实 DeepSeek 与 Blender 端到端回归：Agent 在 revision 14 提交 240 帧、10 秒 IR，地球轨道使用 `target_relative -> sun`，月亮轨道使用 `target_relative -> earth`；全帧距离分别保持在 19.318–20.000 m 与 2.898–3.000 m。规划 hard pass、soft score `1.0`，Blender Runtime Validation 为 0 violation；Workbench preview 输出 120 帧、640×360、12 fps 的 10 秒视频，执行耗时约 4.37 秒。该次规划使用 20 requests、19 tool calls、323,067 aggregate input tokens（其中 287,616 cache read）和 7,672 output tokens；供应商响应记录的 cost 为 `0.023138433`，但未返回币种或价格快照，因此不将其标注为 USD。
