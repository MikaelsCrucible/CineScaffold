...
- 未指定水平机位不再触发固定 35°/55° 偏航：静态或仅状态变化场景沿规范纵深轴观察，有线性主体运动时使用 Agent 的类型化观察选择，未选择则只采用 Validator 冻结的最小可读斜角。明确远景即使只写入 `spatial_layers` 也会归一化为 Planning 的纵深关系。旧情绪表中的固定俯仰、地平线百分比和主体水平位置已退出自动量化；用户明确俯仰会转换成实际机位并硬验证，明确水平画面位置会转换成 `screen_region` hard constraint。地平线百分比因没有独立于机位、焦点和场景几何的稳定语义，正式不作为自动或显式数值接口。
...
- Cinematic Brief v0.7 先区分主体静态/动态场景；动态场景按实体记录稀疏、独立且带数值起止时间的动作区间，并用类型化时间关系表达相接、先后、包含和重叠，不再按全场事件数量机械等分总时长。全流程明确区分“主体状态发生变化”“主体发生空间位移”和“摄影机发生运动”：运镜不能把主体静态场景变成动态，也不能触发主体运动可读性策略；只有 `path_move/carried` 才属于主体空间运动。所有独立位移统一为 `path_move`；公转只由 `relative_to_target + target_id + circle/ellipse` 表达，不再同时提交 `orbit` kind、`orbit_around` direction 或重复 Skeleton Relation。闭合路径的类型化目标本身就是几何证据，不要求重复填写自然语言方向；目标缺失时只允许从同一事件的唯一公转关系确定性补齐。局部互动必须物化为可观察的旋转/尺度变化，不能用静止轨道假装完成；抛物线路径会物化为带弧顶的关键点。稳定 `motion_id` 必须绑定对应的类型化动作来源，不能通过改写 `source_ref` 绕过验证。
...
- Semantic Provider 偶发把同一动作的冗余类型字段写成单项矛盾时，转换层会依据 `action_kind`、`motion_type`、`motion_mode`、路径和载体/容纳关系的唯一多数确定性归一化，不追加付费模型请求；证据打平、引用无效或真正含义不明确时仍失败关闭。摄影机的固定位置或旋转语义不参与主体运动类型表决。
- 复合叙事动作不会扩张为场景专用实体、专用执行原语或隐含几何方向。`action_kind` 只保留 hold/locomotion/interact/other；“驶来、离开、接走、上车”等文字本身不会生成朝向人物、远离人物或撞入载体中心的轨迹。接载点由事件末端空间关系表达，上车可由隐藏/容纳/随载体状态表达。规划器先联合读取同一实体的完整时间线，再为未明确转向的连续运动选取一致路线，静止阶段不会清除既有行进方向；单个会合点本身只产生路径点，不再暗含整条路线禁止转向。等待/停留仍写入保持关键点，且一个已有前置状态的主体不会为了后续动作被倒推到新的首帧位置。已隐藏的被运载代理无需生成重叠的可见跟随轨道。独立主体时间段发生重叠时，Semantic 层会把模型误用的 `before/after/meets` 规范化为与数值区间一致的开始、结束或重叠关系；模型推断的零间隔不会阻止纠正，用户明确给出的数值间隔仍严格校验。
