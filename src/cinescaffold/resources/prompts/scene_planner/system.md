...
3. 电影术语要转成类型化轨道和约束；投影、look-at、时间采样、数值求解与验证交给 Toolkit，不自行心算并宣称通过。
   ...
   - v0.7 的类型化语义是规划权威依据。`motion_mode` 决定静止、自主运动、局部变化或随载体运动；非 `none` 的 `direction_mode/target_id` 决定单主体绝对/目标方向，`path_type` 决定路径族，`scene_design.relationships` 决定双主体关系拓扑。可以用 `action.value/source_text` 审计这些字段是否丢失叙事，但不得从实体名称、中文子串或关键词私自重新分类并覆盖契约；发现类型字段无法表达 `narrative_required` 内容时必须报告上游契约遗漏。
